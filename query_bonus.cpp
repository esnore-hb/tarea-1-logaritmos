...
/**
 * @struct RTreeEntry
 * @brief Entrada en un nodo del R-Tree que mapea un MBR a la posición de un
 * nodo.
 *
 * Representa un par clave-valor donde la clave es el MBR (rectángulo
 * delimitador) y el valor es la posición (índice) del nodo hijo en el archivo
 * de disco. Para nodos hoja, position = -1 indica que es un punto de datos.
 *
 * @member key      - Rectángulo delimitador (MBR) que actúa como clave de
 * búsqueda
...
bool mbrs_intersect(const MBR &mbr1, const MBR &mbr2) {
  ...
  return !(mbr1.x2 < mbr2.x1 || mbr1.x1 > mbr2.x2 || mbr1.y2 < mbr2.y1 ||
           mbr1.y1 > mbr2.y2);
}
...
void searchRectangle(std::fstream *file, int node_pos, const MBR &search_rect,
  ...
  for (int i = 0; i < node.k; ++i) {
    ...
    // Verificar si el MBR de esta entrada intersecta con el rectángulo de
    // búsqueda
 ...
 * @brief Genera conjuntos de cuadrados (MBRs) de diferentes tamaños para
 * consultas.
 *
 * Descripción:
 *     Crea 5 conjuntos de cuadrados, cada uno con un tamaño de lado 's'
 * diferente. Para cada tamaño 's', se generan 100 cuadrados con posiciones
 * aleatorias dentro del rango [0, 1] x [0, 1]. Los cuadrados se usan como
 * rectángulos de búsqueda para realizar consultas de rango en el R-Tree.
 *
 ...
 *         - Los índices corresponden a los valores s: [0.0025, 0.005, 0.01,
 * 0.025, 0.05]
 *         - Cada MBR es un cuadrado con lado de longitud s
 *
 *     Estructura de datos retornada:
 *         all_squares[i][j] = MBR del j-ésimo cuadrado para el tamaño
 * s_values[i] donde all_squares[i] contiene 100 cuadrados y hay 5 vectores en
 * total
 */
std::vector<std::vector<MBR>> generate_squares() {
  ...
  // Inicialización de la estructura de resultados: 5 listas, cada una con 100
  // MBRs.
  ...
  // Usaremos un rango arbitrario grande (ej. [0, 10]) para la posición inicial
  // (x1, y1)
  ...
  for (float s : s_values) {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      // Calcular la segunda esquina (debido a que es un cuadrado, el delta es
      // 's')
      float x2 = x1 + s;
      float y2 = y1 + s;
...
/**
 * @function main
 * @brief Realiza búsquedas de rango en un R-Tree y reporta métricas de
 * rendimiento.
 *
 * Descripción:
